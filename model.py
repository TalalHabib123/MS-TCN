...
class Trainer:
    ...
    # def video_to_npy(self, video_path, sample_rate):
    ...
    #         if not ret:
    #             break
    #         frame = cv2.resize(frame, (224, 224))  # Resize frame to match model input size if necessary
    #         frames.append(frame)
    #     cap.release()
    #     batch_size = 1
    #     # time, height, width, channels = frames.shape
    #     frames = np.array(frames)
    #     frames = frames.transpose((3, 0, 1, 2))  # Change to (C, T, H, W) if needed
    #     import pdb; pdb.set_trace()
    ...
    def video_to_npy(self, video_path, sample_rate):
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            frame = cv2.resize(frame, (32, 64))  # Resize frame to match model input size if necessary
        ...
        flattened_frames = frames.reshape(2048, frames.shape[0])     # channels * height * width
        
        # Ensure the first dimension is 2048
        if flattened_frames.shape[0] != 2048:
            raise ValueError(f"Expected first dimension to be 2048, but got {flattened_frames.shape[0]}")
    ...
    #     batch_size = 1
    #     time, height, width, channels = frames.shape
    #     frames = frames.transpose(3, 0, 1, 2)  # Change to (C, T, H, W)
    #     frames = frames.reshape(channels, -1)  # Flatten spatial dimensions

    #     frames = frames[:, ::sample_rate]  # Sample frames based on the sample_rate
        
    #     return frames


    def inference(self, model_dir, results_dir, video_path, epoch, actions_dict, device, sample_rate):
        print("Inference")
        self.model.eval()
        with torch.no_grad():
            self.model.to(device)
            ...
            features = self.video_to_npy(video_path, sample_rate)
            
            input_x = torch.tensor(features, dtype=torch.float)  # Change dtype to float32
            input_x.unsqueeze_(0)
            input_x = input_x.to(device)
            print("Predicting")
            predictions = self.model(input_x)
